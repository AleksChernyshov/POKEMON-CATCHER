      ...
      <div className="relative mx-auto mt-12 w-[512px] h-[512px] select-none" style={{ backgroundImage: `url(${pokedexImg})`, backgroundSize: 'contain', backgroundRepeat: 'no-repeat' }}>
        {power && (
          <>
            {current && (
              <>
                <img src={current.sprites.front_default} alt={current.name} className="absolute left-[68px] top-[150px] w-[152px] h-[124px] rounded object-contain" />
                <div className="absolute right-[63px] top-[150px] w-[153px] h-[66px] flex items-center justify-center text-accent-yellow text-lg font-bold">{current.name}</div>
                <div className="absolute right-[162px] bottom-[107px] w-[57px] h-[40px] flex items-center justify-center text-white font-bold text-lg">×{current.count}</div>
                <div className="absolute right-[53px] bottom-[108px] w-[90px] h-[38px] flex items-center justify-center text-accent-yellow font-bold text-lg">Stage {current.stage + 1}</div>
              </>
            )}
        ...
        <button onClick={prev} className={`group absolute left-[72px] bottom-[132px] w-[36px] h-[24px] cursor-pointer ${disabled}`}>
          <span className={`absolute inset-0 border-l-2 border-t-2 border-b-2 border-transparent rounded-tl-md rounded-bl-md transition-colors duration-75 ${power && hasPokemon && 'group-hover:border-t-white/60 group-hover:border-l-white/60 group-active:border-b-accent-yellow group-active:border-l-accent-yellow group-active:border-t-transparent'}`} />
        </button>
        <button onClick={next} className={`group absolute left-[112px] bottom-[132px] w-[36px] h-[24px] cursor-pointer ${disabled}`}>
          <span className={`absolute inset-0 border-r-2 border-t-2 border-b-2 border-transparent rounded-tr-md rounded-br-md transition-colors duration-75 ${power && hasPokemon && 'group-hover:border-b-white/60 group-hover:border-r-white/60 group-active:border-t-accent-yellow group-active:border-r-accent-yellow group-active:border-b-transparent'}`} />
        </button>

        <div className="absolute left-[95px] bottom-[168px] flex flex-col items-center gap-1 w-[60px] h-[45px]">
          <button onClick={() => power && hasPokemon && setShowAll(true)} className={`group relative w-[46px] h-[10px] cursor-pointer ${disabled}`}>
            <span className={`absolute inset-0 rounded-md border-2 border-transparent transition-colors duration-50 ${power && hasPokemon && 'group-hover:shadow-[0_0_6px_2px_rgba(0,128,255,0.8)] group-active:border-blue-500'}`} />
          </button>
        ...
        <div className="absolute left-[155px] bottom-[168px] flex flex-col items-center gap-1 w-[60px] h-[45px]">
          <button onClick={() => power && hasPokemon && removeOne(current!.id)} className={`group relative w-[46px] h-[10px] cursor-pointer ${disabled}`}>
            <span className={`absolute inset-0 rounded-md border-2 border-transparent transition-colors duration-50 ${power && hasPokemon && 'group-hover:shadow-[0_0_6px_2px_rgba(255,0,0,0.8)] group-active:border-red-500'}`} />
          </button>
        ...
        <div className="absolute left-[62px] bottom-[189px] w-[26px] h-[26px]">
          <button onClick={() => setPower(p => !p)} className="group relative w-full h-full rounded-full cursor-pointer">
